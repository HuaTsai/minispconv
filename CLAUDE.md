...
## 版控：現階段直接 commit 到 main，不開 branch

初始開發階段**不開 feature branch** —— 直接 commit 到 `main` 並推上去。

branch 買到的是「隔離未完成的變更，讓別人不受影響」。現在沒有別人，也沒有需要保持
可用的發布版本，這個代價換到的是零。**未完成的東西直接進 main 是可以的** ——
`random_point_cloud` 就是以「契約寫成失敗的測試」的形式進去的。

**這條什麼時候失效**（出現任一項就恢復開 branch）：

- 有第二個人在這個 repo 上工作
- `main` 開始被當成「可用版本」引用（README 寫了安裝指令、有人 clone 來跑）
- CI 開始跑，且綠燈是 merge 的前提
- 單一變更大到無法一次 commit 收尾（例如 `L2.3` 的 hash table）

**寫下失效條件是這條規則的一半。** 沒有它，「初始階段」會無限延長，
等到真的該開 branch 的時候，這行字會變成理由。
